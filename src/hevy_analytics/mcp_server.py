...
from .bootstrap import get_service
from .config import DEFAULT_RECENT_WORKOUTS_LIMIT
from .response import render_markdown
from .tools import (
...
def create_mcp_server() -> Any:
    ...
    def tool_weekly_volume(weeks: int = 4) -> str:
        service = get_service()
        return render_markdown(
            service.execute("weekly_volume", lambda w: weekly_volume(service, w), weeks)
        )

    @mcp.tool()
    def tool_fatigue_check() -> str:
        service = get_service()
        return render_markdown(service.execute("fatigue_check", lambda: fatigue_check(service)))

    @mcp.tool()
    def tool_suggest_accessories() -> str:
        service = get_service()
        return render_markdown(
            service.execute("suggest_accessories", lambda: suggest_accessories(service))
        )
    ...
    def tool_training_log(days: int = 30) -> str:
        service = get_service()
        return render_markdown(
            service.execute("training_log", lambda d: training_log(service, d), days)
        )
    ...
    def tool_get_routines() -> str:
        service = get_service()
        return render_markdown(service.execute("get_routines", lambda: get_routines(service)))
